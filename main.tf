...
resource "azurerm_virtual_machine" "windows_instance" {
  ...
  os_profile {
    ...
    admin_password = "${coalesce(var.admin_password, random_password.password.*.result[count.index])}"
...
resource "azurerm_virtual_machine_extension" "winrm_setup" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://raw.githubusercontent.com/sergiimatus/terraform-azurerm-windows-instance/master/winrm_setup.ps1"],
